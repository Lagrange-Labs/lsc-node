...
func TestConvertProtobufToMongo(t *testing.T) {
	...
	require.Equal(t, block.ChainHeader.BlockNumber, mBlock["chain_header"].(primitive.M)["block_number"])
...
func TestConvertMongoToBlock(t *testing.T) {
	...
	require.Equal(t, m["chain_header"].(bson.M)["block_number"], int64(block.ChainHeader.BlockNumber))
